...
under $k$; given the same key \ALG{Dec} will invert \ALG{Enc}, and so the
same $m$ can be recovered from the associated ciphertext $c$.  An AES 
parameter set~\cite[Figure 4]{FIPS:197}
is a triple
$
\TUPLE{ Nk, Nb, Nr }
$
...
fixed to $128$.  From here on, we focus wlog. on encryption using AES-128
(other parameter sets are catered for naturally, and decryption with only
minor differences) and so will use the term AES as a synonym for AES-128.
...
\item \AESFUNC{MixColumns}
      ...
      \AESFUNC{MixColumn}
      to denote
      the operation applied to a single column within \AESFUNC{MixColumns},
